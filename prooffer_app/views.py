...
def projectList(request):
	...
	userList = []
        projList = []
	for user in usersdb:
		user_object = usersdb[user]
		
		userList.append({str(user):str(user_object)})
	
        for row in projectsdb.query(request.REQUEST['map_fun']):
	    
		print('lastmodifiedby user')

		usersDbQuery = 'function(doc){if(doc.datatype == /"user/" && doc.code == '+row.value.lastmodifiedby+') {emit(doc.name, {id:doc._id, code:doc.code,privileges: doc.privileges, machines: doc.machines});}}'
           	projList.append({'key':row.key, 'value':row.value, 'userList':usersdb.query(usersDbQuery)})
